...
export function registerHandlers(io: Server, socket: Socket): void {
  ...
  socket.on('session:join', async (data: { sessionId: string }) => {
    try {
      ...
      socket.emit('error', { message: '세션 입장 중 오류가 발생했습니다' });
    }
  });
  ...
  socket.on('caption:send', async (data: { sessionId: string; text: string }) => {
    try {
      ...
      console.error('[Socket] caption:send error:', error);
      socket.emit('error', { message: '자막 송출 중 오류가 발생했습니다' });
    }
  });
  ...
  socket.on('operator:switch', async (data: { sessionId: string; operatorUserId: string }) => {
    try {
      ...
      console.error('[Socket] operator:switch error:', error);
      socket.emit('error', { message: '송출자 전환 중 오류가 발생했습니다' });
    }
  });
  ...
  socket.on('operator:request', async (data: { sessionId: string }) => {
    try {
      ...
      console.error('[Socket] operator:request error:', error);
      socket.emit('error', { message: '교대 요청 중 오류가 발생했습니다' });
    }
  });
...
async function leaveSession(io: Server, socket: Socket, sessionId: string, user: AuthPayload): Promise<void> {
  try {
    ...
    console.error('[Socket] leaveSession error:', error);
  }
}
