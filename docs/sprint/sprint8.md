...
| `server/src/middleware/auth.test.ts` | JWT 인증 단위 테스트 |
| `server/src/socket/handlers.test.ts` | 소켓 핸들러 단위 테스트 |
| `server/src/routes/auth.test.ts` | 인증 API 통합 테스트 |
| `server/src/routes/session.test.ts` | 세션 CRUD/자막/내보내기 통합 테스트 |
...
| 파일 | 변경 |
|------|------|
| `src/renderer/stores/appStore.ts` | 자막 디바운싱 (50ms WebSocket) |
